...
module.exports = function(customerId){
	...
	if(!customer) return{ error:'Unknow customer ID: '+req.params.customerId};
	var orders = customer.getOrders(customerId).map(function(Order){
		return {
	...
	return {
			...
			zip:customer.zip,
			fullAddress:smartJoin([customer.address1,customer.address2,
